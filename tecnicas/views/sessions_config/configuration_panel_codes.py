...
def configurationPanelCodes(req: HttpRequest):
    ...
    if not req.session["form_basic"]:
        ...
        return redirect(reverse(url_main) + "?error=datos del formulario requerido no encontrados")
    ...
    if req.method == "GET":
        if name_technique == "escalas":
            response = PanelCodesController(url_main=url_main).controllGetEscalas(
                req, data_basic)

        elif name_technique in ["rata", "cata", "perfil flash", "sort", "napping", "perfil_ideal"]:
            response = PanelCodesController(url_main=url_main).controllGetNoOrders(
        ...
        if name_technique == "escalas":
            response = PanelCodesController(url_main=url_main).controllPostEscalas(
                req, data_basic)

        elif name_technique in ["rata", "cata", "perfil_ideal"]:
            response = PanelCodesController(url_main=url_main).controllPostNoOrders(
                request=req, name_technique=name_technique)

        elif name_technique in ["perfil flash", "sort", "napping"]:
            response = PanelCodesController(url_next="cata_system:creando_sesion", url_main=url_main).controllPostNoOrders(
                request=req, name_technique=name_technique)
